...
bool CTable::add_column(const char* column_name) {
    ...
    char** new_columns_names = new char* [m_columns_number_ * sizeof(char*)];

    for (unsigned int i = 0; i < m_columns_number_ - 1; i++) {
    ...
    m_columns_names_[m_columns_number_ - 1] = new char[size];
    strncpy_s(m_columns_names_[m_columns_number_ - 1], size, column_name, 256);
...
std::istream& operator>>(std::istream& is, CTable& table) {
    for (unsigned int i = 0; i < table.m_rows_number_; i++) {
        delete[] table.m_data_[i];
    }
    delete[] table.m_data_;

    for (unsigned int i = 0; i < table.m_columns_number_; i++) {
        delete[] table.m_columns_names_[i];
    }
    delete[] table.m_columns_names_;
